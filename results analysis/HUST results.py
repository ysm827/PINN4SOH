...
if __name__ == '__main__':
    root = './results/HUST results/'
    writer = pd.ExcelWriter('./results/HUST_results.xlsx')
    results = Results(root)

    df_battery_mean = results.get_battery_mean()
    df_experiment_mean = results.get_experiments_mean()
    df_battery_mean.to_excel(writer, sheet_name='battery_mean', index=False)
    df_experiment_mean.to_excel(writer, sheet_name='experiment_mean', index=False)
